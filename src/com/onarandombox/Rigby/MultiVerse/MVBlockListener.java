...
public class MVBlockListener extends BlockListener {
    ...
    public void onBlockPlace(BlockPlaceEvent event) {
        ...
        if (portal != null) {
            ...
            if (!p.getOwner().equals(event.getPlayer().getName()) && !MultiVerse.Permissions.has(event.getPlayer(), "multiverse.portal.override")) {
                event.setCancelled(true);
            }
        }
    }
    ...
    public void onBlockBreak(BlockBreakEvent event) {
        ...
        if (portal != null) {
            MVPortal p = this.plugin.MVPortals.get(portal);
            if (!p.getOwner().equals(player.getName()) && !MultiVerse.Permissions.has(player, "multiverse.portal.override")) {
                event.setCancelled(true);
            }
        }
    }
    ...
    public void onBlockDamage(BlockDamageEvent event) {
        ...
        if (player.getItemInHand().getTypeId() == this.plugin.configMV.getInt("setwand", 270) && MultiVerse.Permissions.has(player, "multiverse.portal.create")) {
            ...
            Location l = event.getBlock().getLocation();
            if (this.plugin.playerSessions.get(player.getName()).compareLocation1(l)) {
                return;
            }
            ...
            if (!(portal == null)) {
                ...
                if (p.getDestLocation() != null) {
                    ...
                    if (s[0].equalsIgnoreCase("P")) {
                        player.sendMessage("Destination Portal - " + s[1].toString());
                    }
                    if (s[0].equalsIgnoreCase("W")) {
                        player.sendMessage("Destination World - " + s[1].toString());
                    }
                }
                ...
                this.plugin.playerSessions.get(player.getName()).setInfoCooldown();
            }
        }
        /**
         * End
