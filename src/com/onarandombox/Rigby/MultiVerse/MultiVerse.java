...
public class MultiVerse extends JavaPlugin {
	/**
	 * Variable to contain the CraftBukkit version which this was made for and servers should not use anything less than this.
	 */
	private final int CBVer = 320;
	/**
	 * Variables to contain checks on the external plugins, so far this is just Permissions and iConomy. Permissions is REQUIRED.
	 */
	public static PermissionHandler Permissions;
	public static boolean useiConomy = false;
	/**
	 * Setup the Logger, also set a public variable which contains the prefix for all log messages, this allows for easy change.
	 */
	private final Logger log = Logger.getLogger("Minecraft");
	public final String logPrefix = "[MultiVerse] ";
	/**
	 * Setup all the Configuration variables... first 3 are actual config files to read write to, 4 one is the process we run to setup the initial files.
	 */
	...
	/**
	 * HashMap to contain all player session related stuff, eg what coordinates they have selected or what portal they have selected.
	 */
	...
	public void onEnable() {
		...
		/**
		 * First we run our 'confSetup' class to check if the Configuration files exist. If either of them is missing it will create it with a empty template.
		 */
		...
		configMV.load();
		/**
		 * Check whether iConomy is enabled, then check whether they want players to be charged for using Portals.
		 */
		if (checkiConomy()) {
		...
		configPortals.load();
		/**
		 * Now we run the functions to place each world and portal into our HashMap with our Custom Class.
		 */
		loadWorlds();
		loadPortals();
		/**
	...
	/**
	 * This section is run once when the Plugin is enabled, this will load all the Worlds and place them into the HashMap & Custom Class.
	 */
	public void loadWorlds() {
		...
		int count = 0;
		/**
		 * Grab all the Worlds from the 'worlds.yml' we already loaded this file into 'configWorlds' during the onEnable stage.
		 */
		...
		if (worldKeys != null) {
			...
			for (String worldKey : worldKeys) {
				 ...
				 */
				if (!worldKey.matches(MVCommands.WORLD_NAME_REGEX)) {
				...
				/**
				 * Tell the server to create the world then place it within a variable so we can pass this onto our HashMap.
				 */
				...
				/**
				 * Place the World into hour HashMap with our Custom Class, the custom class also gets passed along the config file so it can edit it and save it, as well as this class.
				 */
		...
		/**
		 * If the config file does not contain our default world then we will load it anyways this is needed because all commands based on Worlds are checked via the HashMap.
		 */
		if (!this.MVWorlds.containsKey(getServer().getWorlds().get(0).getName())) {
			...
			/**
			 * Place the World into our HashMap and then force a save of the config file, this makes it so the default world and it's settings are then saved into the config.
			 */
		...
		/**
		 * Load the Nether world the same way. I don't want to, but this update needs to ship, it'll be better in 2.X. I promise
		 */
		String netherName = getServer().getWorlds().get(0).getName() + "_nether";
		if (!this.MVWorlds.containsKey(netherName)) {
			/**
			 * Grab the World at ID 0, this will always be the default world.
			 */
			World world = getServer().getWorld(netherName);
			if (world != null) {
				/**
				 * Another simple Output.
				 */
	...
	/**
	 * This section is run once when the Plugin is enabled, this will load all the Portals and place them into the HashMap & Custom Class.
	 */
	private void loadPortals() {
		...
		int count = 0;
		/**
		 * Loop through all the entries within the list and load the settings into our HashMap with its Custom Class.
		 */
	...
	/**
	 * Simple Script which runs when the Plugin is disabled... Currently we just use this to output to the user the fact it's disabled and to stop the updateChecker if one was initialised. We can't unregister events atm.
	 */
	@Override
	public void onDisable() {
		/**
		 * If updateCheck is NOT NULL then it means we have enabled Update Checking, this needs to be cancelled otherwise even though the plugin is disabled they will still get alerts telling them it's out of date.
		 */
