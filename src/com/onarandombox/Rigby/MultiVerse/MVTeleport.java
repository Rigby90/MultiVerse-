...
public class MVTeleport {
        ...
        while (notSafe) {
            ...
            for (int xx = x - distance; xx <= x + distance; xx++) {
                for (int zz = z - distance; zz <= z + distance; zz++) {
                    if (this.plugin.utils.isPortal(new Location(world, xx, y, zz, (float) 0.0, (float) 0.0)) == null) {
                        if (!blockIsNotSafe(world, xx, y, zz) && d == null) {
                            Double dx = xx + 0.5;
                            Double dz = zz + 0.5;
                            notSafe = false;
                            d = new Location(world, dx, y, dz, l.getYaw(), l.getPitch());
                            break;
                        }
                    }
                }
        ...
        if (p.getDestLocation() != null) {
            ...
            if (dest[0].toString().equalsIgnoreCase("P")) {
                if (this.plugin.MVPortals.containsKey(dest[1].toString())) {
                    ...
                    Vector v = pd.min;
                    Location l = new Location(pd.getWorld(), v.getX(), v.getY(), v.getZ(), (float) 0.0, (float) 0.0);
            ...
            if (dest[0].toString().equalsIgnoreCase("W")) {
                World world = this.plugin.getServer().getWorld(dest[1].toString());
                if (world != null) {
    ...
    public Location portalSignMethod(Player pl) {
        ...
        Block b = pl.getWorld().getBlockAt(l.getBlockX(), l.getBlockY(), l.getBlockZ());
            ...
            if (s.size() != 0) {
                for (int i = 0; i < s.size(); i++) {
                    ...
                    if ((sign.getLine(1).equalsIgnoreCase("[multiverse]") || sign.getLine(1).equalsIgnoreCase("[mv]")) && sign.getLine(2).length() > 0) {
                        ...
                        if (world != null) {
                            return getDestination(this.plugin.MVWorlds.get(world.getName()).getSpawnLocation(), pl);
                        }
                    }
                }
            }
    ...
    public Location portalSPNether(Player pl) {
        ...
        if (this.plugin.getServer().getWorld(nether) == null || this.plugin.getServer().getWorld(world) == null) {
            return null;
        }
        ...
        if (b.getType().equals(Material.PORTAL)) {
            ...
            World w;
            Location p;
            if (pl.getWorld().getEnvironment() == Environment.NORMAL && pl.getWorld().getName().equalsIgnoreCase(world)) {
                w = netherw;
                x = x / factor;
                z = z / factor;
                l = new Location(w, x, y, z);
                p = nearbyPortal(l, pl);
            } else if (pl.getWorld().getEnvironment() == Environment.NETHER && pl.getWorld().getName().equals(nether)) {
                w = worldw;
                x = x * factor;
                z = z * factor;
                l = new Location(w, x, y, z);
