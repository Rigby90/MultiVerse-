...
public class MVPlayerListener extends PlayerListener {
	...
	public void onPlayerRespawn(PlayerRespawnEvent event) {
		...
		if (this.configuration.getBoolean("globalrespawn", false)) {
			l = playerTeleporter.getDestination(
		 ...
		 */
			...
			if (this.configuration.getBoolean("alternaterespawn", false)) {
				if (!(world.getName().equals(this.plugin.getServer()
						.getWorlds().get(0).getName()))) {
					l = playerTeleporter.getDestination(this.plugin.MVWorlds
							.get(world.getName()).getSpawnLocation(), player);
				}
			}
		}
		/**
		...
		/*if (event.getPlayer().getWorld().getName()
				.equalsIgnoreCase(this.configuration.getString("nether"))) {
			if (this.configuration.getBoolean("respawntodefault", true)
					&& this.configuration.getBoolean("splike", false)) {
				l = this.plugin.MVWorlds.get(
	...
	public void onPlayerChat(PlayerChatEvent event) {
		...
		if (this.configuration.getBoolean("prefix", false)) {
			 ...
			 */
			if ((this.plugin.MVWorlds.get(world).getAlias() != "")
	...
	public void onPlayerMove(PlayerMoveEvent event) {
		...
		if (d != null) {
			...
			if (!(playerTeleporter.canTravelFromWorld(pl, d.getWorld()))) {
				ps.sendMessage("Sorry but you cannot travel to '"
						+ d.getWorld().getName() + "' from this World!");
				return;
			}
			if (!(playerTeleporter.canEnterWorld(pl, d.getWorld()))) {
				ps.sendMessage("Sorry but you cannot enter the '"
						+ d.getWorld().getName() + "' world.");
				return;
			}
			/*
